...
// A cached copy of an older script against newer markup would leave dead controls; say so instead of failing silently.
if(!generate||!download||!status||!preview||!reportTitle||slots.some(slot=>!slot.file||!slot.title)) {
 const target=status||document.body;
 target.textContent='This page and its scripts are out of step. Reload with Ctrl+F5 (Cmd+Shift+R on macOS) to clear the cached version.';
} else {
